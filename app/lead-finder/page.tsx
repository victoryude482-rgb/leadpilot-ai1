...
export default function LeadFinderPage() {
  ...
  async function researchNow(event: FormEvent) { event.preventDefault(); if(!question.trim()) return; setResearching(true); setResearch({}); try { setResearch(await researchQuestion(question)); } catch(error){ setResearch({error:error instanceof Error?error.message:'Research failed'}); } finally { setResearching(false); } }

  return <main style={{maxWidth:960,margin:'0 auto',padding:32,fontFamily:'system-ui'}}>
    <h1>LeadPilot AI</h1>
    <p>Search leads or ask the AI to research a topic. It plans several searches, checks multiple public sources, compares evidence, and shows citations instead of pretending it knows.</p>

    <section style={{marginTop:24,padding:18,border:'1px solid #ddd',borderRadius:14}}>
      <h2>AI Research</h2>
      <p style={{opacity:.7}}>Perplexity-style multi-source research across web results, news and Reddit.</p>
      ...
      {research.answer && <div style={{marginTop:16,padding:14,background:'#f6f6f6',borderRadius:10}}><strong>AI answer</strong><p>{research.answer}</p></div>}
      ...
      {research.sources?.length ? <div style={{marginTop:16}}><h3>Sources ({research.sources.length})</h3>{research.sources.map((s,i)=><article key={`${s.url}-${i}`} style={{padding:'10px 0',borderBottom:'1px solid #eee'}}><a href={s.url} target="_blank" rel="noreferrer"><strong>{s.title}</strong></a><div style={{fontSize:12,opacity:.7}}>{s.source}{s.publishedAt?` · ${s.publishedAt}`:''}</div>{s.snippet&&<div style={{fontSize:13,marginTop:4}}>{s.snippet.slice(0,300)}</div>}</article>)}</div>:null}
