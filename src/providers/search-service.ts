...
async function searchProviderFast(provider: LeadProvider, query: LeadSearchQuery): Promise<DiscoveredBusiness[]> {
  const timeoutMs = providerTimeoutMs(provider); let timer: ReturnType<typeof setTimeout> | undefined;
  try { return await Promise.race([provider.search(query), new Promise<never>((_, reject) => { timer = setTimeout(() => reject(new Error(`provider timed out after ${timeoutMs / 1000}s`)), timeoutMs); timer.unref?.(); })]); }
...
function tokens(value?: string): string[] {
  return (value || '').toLowerCase().replace(/[^a-z0-9]+/g, ' ').split(/\s+/).filter((t) => t.length >= 3 && !/^\d+$/.test(t) && !STOP_WORDS.has(t));
}
...
function rank(records: DiscoveredBusiness[], query?: LeadSearchQuery): DiscoveredBusiness[] {
  return [...records].sort((a, b) => {
    const q = query ? [...tokens(query.industry), ...tokens(query.keywords)] : [];
    const score = (record: DiscoveredBusiness) => {
      const haystack = [record.name, record.industry, record.address].filter(Boolean).join(' ').toLowerCase();
      const relevance = q.reduce((n, token) => n + (variants(token).some((v) => haystack.includes(v)) ? 5 : 0), 0);
...
function broadenQuery(query: LeadSearchQuery): LeadSearchQuery {
  const cleaned = (query.keywords || '').replace(/\b(find|show|give|tell me|look for|search for|what are|what is|\d+)\b/gi, ' ').replace(/\s+/g, ' ').trim();
  return { ...query, keywords: [query.industry, cleaned, query.city, query.country].filter(Boolean).join(' ').trim() || 'business' };
}
export async function searchLeads(providers: LeadProvider[], query: LeadSearchQuery): Promise<SearchResult> {
  ...
  const run = async (q: LeadSearchQuery) => {
    ...
    return settled.flatMap((result, index) => { if (result.status === 'fulfilled') return result.value; const provider = enabled[index]; warnings.push(`${provider.constructor.name}: ${result.reason instanceof Error ? result.reason.message : 'search failed'}`); return []; });
  };
  let records = await run(query); const before = records.length; records = records.filter((record) => relevant(record, query));
  if (before > records.length) warnings.push(`${before - records.length} unrelated provider records were rejected.`);
  if (records.length === 0) {
    const broaderRaw = await run(broadenQuery(query)); records = broaderRaw.filter((record) => relevant(record, query));
    if (broaderRaw.length && !records.length) warnings.push('Providers returned businesses, but none matched the requested business type. No unrelated leads were substituted.');
  }
