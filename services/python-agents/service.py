...
def lead_finder(payload:dict[str,Any])->dict[str,Any]:
 ...
 for b in businesses:
  ...
  if industry and industry!="business": signals.append("industry classified")
  else: missing.append("industry classification")
 ...
 return {"agent":"lead-finder","results":out,"warnings":[],"strategy":["TypeScript owns discovery, verification, scoring, persistence and recovery.","Python owns evidence interpretation and lead enrichment only.","Python never invents missing contact details or verified business facts."]}
def website_brand(payload:dict[str,Any])->dict[str,Any]:
 query=str(payload.get("query","")).strip() or "business website"; context=str(payload.get("context","")).strip(); q=(query+" "+context).lower()
 if re.search(r"restaurant|food|cafe|bar|hotel",q): industry="hospitality"
 elif re.search(r"real estate|property|realtor",q): industry="professional services"
 elif re.search(r"plumb|hvac|roof|contractor|construction|cleaning",q): industry="local service"
 elif re.search(r"agency|marketing|photograph|design|beauty|fashion",q): industry="creative"
 elif re.search(r"saas|software|app|ai|tech",q): industry="software"
 else: industry="business"
 ...
 return {"agent":"website-brand","results":[{"name":query,"industry":industry,"brandDirection":{"personality":personality,"tone":"conversational, specific and customer-first","avoid":["invented claims","generic AI buzzwords","fake testimonials","unsupported statistics"]},"websitePlan":{"pages":["Home","Services","About","FAQ","Contact"],"sections":["clear value proposition","proof/evidence","services","customer questions","strong next step"],"mobileFirst":True},"copy":{"headline":f"A clearer way to choose {query}","subheadline":f"A human, practical website experience designed around what customers need to know about {query}.","cta":"Talk to us"},"designDecisions":["Use one strong visual hierarchy rather than crowded sections.","Keep navigation simple and task-oriented.","Use real business evidence instead of invented claims.","Make the primary customer action obvious on mobile."]}],"warnings":[],"strategy":["Python owns industry inference, brand direction, copy decisions and information architecture.","TypeScript remains responsible for rendering, persistence, export and side effects.","Unknown business facts are intentionally left for the owner to supply."]}
def generic(payload):
 ...
 if agent=="gbp-fix": return {"agent":agent,"results":[{"plan":["Review the supplied listing evidence and define the highest-impact fixes."]}],"warnings":[]}
