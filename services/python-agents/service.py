...
REASONING_AGENTS={"lead-finder","trend-finder","opportunity-finder","tender-finder","ecommerce-opportunity","workpilot","content","competitor-monitor","outreach","website-brand","gbp-audit","gbp-outreach","gbp-fix","general-manager"}
...
def lead_finder(payload:dict[str,Any])->dict[str,Any]:
 businesses=payload.get("businesses") if isinstance(payload.get("businesses"),list) else []; query=str(payload.get("query","")).strip(); out=[]
 for b in businesses:
  if not isinstance(b,dict): continue
  name=str(b.get("name") or "business"); industry=str(b.get("industry") or payload.get("industry") or "business"); city=str(b.get("city") or payload.get("city") or ""); website=str(b.get("website") or ""); signals=[]; missing=[]
...
def website_brand(payload):
 query=str(payload.get("query","")).strip() or "business website"; q=query.lower(); industry="hospitality" if re.search(r"restaurant|food|cafe|bar|hotel",q) else "professional services" if re.search(r"real estate|property|realtor",q) else "local service" if re.search(r"plumb|hvac|roof|contractor|construction|cleaning",q) else "creative" if re.search(r"agency|marketing|photograph|design|beauty|fashion",q) else "software" if re.search(r"saas|software|app|ai|tech",q) else "business"; personality={"hospitality":["warm","inviting","visual"],"professional services":["credible","clear","confident"],"local service":["practical","trustworthy","direct"],"creative":["distinctive","human","expressive"],"software":["clear","modern","focused"],"business":["professional","human","clear"]}[industry]
...
def run(payload):
 ...
 if agent=="general-manager":
  query=str(payload.get("query","")); requested=payload.get("agents") if isinstance(payload.get("agents"),list) else []; agents=choose_agents(query,requested); context={k:v for k,v in payload.items() if k not in {"query","agents","account_id","agent","results"}}; return build_response(query,agents,context,payload.get("results") if isinstance(payload.get("results"),list) else [])
...
class Handler(BaseHTTPRequestHandler):
 ...
 def do_POST(self):
  if self.path not in {"/run","/v1/agents/general-manager"}:return self.send_json(404,{"error":"not found"})
  try:
   n=int(self.headers.get("Content-Length","0"));payload=json.loads(self.rfile.read(n) or b"{}")
   if self.path=="/v1/agents/general-manager": payload["agent"]="general-manager"
   result=run(payload);self.send_json(200 if "error" not in result else 400,result)
